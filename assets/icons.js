/**
 * AKASHA LIBRARY — SVG Icon System (Vanilla JS)
 * Line icons, 24x24 viewBox, currentColor stroke.
 *
 * Usage (ES module):
 *   import { icon, ICONS } from './icons.js';
 *   el.innerHTML = icon('search', 20, 1.6);
 *
 * Usage (script tag):
 *   <script src="assets/icons.js"></script>
 *   el.innerHTML = window.akashaIcons.icon('search');
 */
 ...
 * @param {number} [stroke=1.6] - Stroke width
 * @returns {string} SVG markup string
 */
function icon(name, size = 20, stroke = 1.6) {
  const path = ICONS[name];
  if (!path) return '';
  return `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="${stroke}" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0">${path}</svg>`;
}
