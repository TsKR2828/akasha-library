...
| Step | 功能 | 狀態 |
|------|------|------|
| 4.1 | 文字擷取層（pdf.js 抽文字 / Tesseract.js OCR） | ⬜ 待做 |
| 4.2 | Embedding 索引（本地 transformers.js 或 API） | ⬜ 待做 |
| 4.3 | RAG 檢索（cosine similarity 找相關頁） | ⬜ 待做 |
| 4.4 | LLM 對話 proxy（Cloudflare Workers） | ⬜ 待做 |
| 4.5 | 聊天 UI（側邊面板、對話紀錄） | ✅ 骨架完成 |
| 4.6 | 月幣系統（餘額、扣款、報價預估） | ⬜ 待做 |
| 4.7 | BYOK 模式（使用者自帶 API Key） | ⬜ 待做 |
...
| 5.2 | 隱私權政策頁 + 應用首頁 | ⬜ 待做 |
| 5.3 | GitHub Pages 部署 + 自訂網域（選配） | ⬜ 待做 |
